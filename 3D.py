...
# 加载YOLOv5模型
model = torch.hub.load('ultralytics/yolov5', 'custom', path='exp8/weights/best.pt')
...
try:
    while True:
        ...
        for i, detection in enumerate(results.xyxy[0]):
            x1, y1, x2, y2, conf, cls = detection # 左上角和右下角的坐标信息、置信度信息、类别信息（像素坐标系）
        ...
        if max_index != -1:
            ...
            print(f"目标：{class_name}，置信度：{conf}，深度：{object_depth}，位置：{object_position}")

            # 绘制边界框和类别标签
            cv2.rectangle(color_image, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
            cv2.putText(color_image, f"{class_name} {conf:.2f}", (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
